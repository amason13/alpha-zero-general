...
class ConvLSTM():
    def __init__(self, game, args):
        ...
        h_conv2 = Activation('relu')(BatchNormalization(axis=3)(ConvLSTM2D(args.num_channels, 3, padding='same', use_bias=False, return_sequences=True)(h_conv1)))         # batch_size  x board_x x board_y x num_channels
        h_conv3 = Activation('relu')(BatchNormalization(axis=3)(ConvLSTM2D(args.num_channels, 3, padding='valid', use_bias=False, return_sequences=True)(h_conv2)))        # batch_size  x (board_x-2) x (board_y-2) x num_channels
        h_conv4 = Activation('relu')(BatchNormalization(axis=3)(ConvLSTM2D(args.num_channels, 3, padding='valid', use_bias=False, return_sequences=True)(h_conv3)))        # batch_size  x (board_x-4) x (board_y-4) x num_channels
        h_conv4_flat = Flatten()(h_conv4)       
        s_fc1 = Dropout(args.dropout)(Activation('relu')(BatchNormalization(axis=1)(Dense(1024, use_bias=False)(h_conv4_flat))))  # batch_size x 1024
        s_fc2 = Dropout(args.dropout)(Activation('relu')(BatchNormalization(axis=1)(Dense(512, use_bias=False)(s_fc1))))          # batch_size x 1024
        self.pi = Dense(self.action_size, activation='softmax', name='pi')(s_fc2)   # batch_size x self.action_size
